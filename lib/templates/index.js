...
const generateFlowFromTemplate = function(templateId, templateVersion, flowName, callback) {
...
const getKeys = function(template) {
  let keys = utils.getFromBetween.get(template, '%%', '%%');
...
const updateTemplatesFile = function(templates) {
  fs.writeFileSync(path, JSON.stringify(templates));
}
